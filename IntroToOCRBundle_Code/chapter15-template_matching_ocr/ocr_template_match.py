...
ap.add_argument("-r", "--reference", type=str,
    default="IntroToOCRBundle_Code/chapter15-template_matching_ocr/ocr_a_reference.png",
...
if ref is None:
    print(f"[ERROR] Failed to load reference image {args['reference']}.")
    sys.exit(1)
...
if image is None:
    print(f"[ERROR] Failed to load input image {args['image']}.")
    sys.exit(1)
...
cv2.imwrite("output_tophat.png", tophat)
...
cv2.imwrite("output_gradient.png", grad)
...
cv2.imwrite("output_thresh.png", thresh)
